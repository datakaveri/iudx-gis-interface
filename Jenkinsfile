pipeline {
  ...
  stages {
    ...
    stage('Run GIS interface server'){
      steps{
        script{
            sh 'scp src/test/resources/IUDX_GIS_Server_APIs_V5.5.0.postman_collection.json jenkins@jenkins-master:/var/lib/jenkins/iudx/gis/Newman/'
      ...
      steps{
        node('built-in') {
          script{
            ...
            sh 'HTTP_PROXY=\'127.0.0.1:8090\' newman run /var/lib/jenkins/iudx/gis/Newman/IUDX_GIS_Server_APIs_V5.5.0.postman_collection.json -e /home/ubuntu/configs/5.5.0/gis-postman-env.json -n 2 --insecure -r htmlextra --reporter-htmlextra-export /var/lib/jenkins/iudx/gis/Newman/report/report.html --reporter-htmlextra-skipSensitiveData'
            runZapAttack()
          }
        }
      }
